...
class Countdown extends React.Component {
    state = {
        ...
        seconds: undefined,
        countdownCopy: 510,
        POLL_LENGTH: 10,                                                             // Change this // 2-min or 10 min
    }
    componentDidMount() {
        ...
        this.interval = setInterval(() => {
            ...
            this.state.countdownCopy > 950 &&                                       // Change this // 2-Min = 150 // 10-min = 950
            ...
            this.state.countdownCopy <= 505 &&                                         // Change this // 2-min=105 // 10-min 505
                this.state.countdownCopy >= 455 &&                                     // Change this  // 2-min=55 // 10-min 455   
                this.props.dispatch({
                    type: 'FETCH_STATUS',
                    payload: { url: this.props.route }
                })
            ...
            this.state.countdownCopy === 459                                         // Change this  // 2-min=59 // 10-min 459
                ...
                {moment().utc().diff(moment(this.props.time)) / 60000 < this.state.POLL_LENGTH &&
                    <div className="sticky-timer">
                        ...
                        {this.state.countdownCopy < 200 && this.state.countdownCopy >= 100 &&
                            <p>Just {minutes.slice(1)}:{seconds} left</p>}
                        {this.state.countdownCopy < 100 &&
                            <p>Don't forget to lock in your vote<br/>Just {seconds} seconds remain</p>}
                    </div>}
            </>
        );
    }
