...
function* addIdeaSaga(action) {
    ...
    try {
        if(verbose) console.log("In addIdeaSaga with", action.payload);
        yield axios.post('/api/idea/', action.payload);
        yield put({type:'GET_FULL_IDEA_LIST', payload: {id: action.payload.poll_id}})
    } catch (error) {
        if(verbose) console.log("addIdeaSaga error was", error)
    }
}
...
function* deleteIdeaSaga(action) {
    ...
    try {
        if(verbose) console.log("In deleteIdeaSaga with", action.payload);
        yield axios.delete(`/api/idea/${action.payload.idea_id}/${action.payload.voter_id}`)
        yield put({type:'GET_FULL_IDEA_LIST', payload: {id: action.payload.poll_id}})
    } catch (error) {
        if(verbose) console.log("deleteIdeaSaga error was",error);
    }
}
...
function* getIdeasSaga(action) {
    ...
    try {
        if(verbose) console.log("In getIdeasSaga with", action.payload)
        ...
        if(verbose) console.log('Error in getIdeasSaga',error);
...
function* updateIdeaSaga(action) {
    try {
        ...
        if(verbose) console.log('Error in updateIdeaSaga',error);
    }
}
function* rootSaga() {
